; Technology File RERAM
; Generated on May  2 13:49:40 2017
;     with @(#)$CDS: virtuoso version 6.1.6-64b 08/12/2015 21:31 (sjfnl165) $
...
layerDefinitions(
 ...
 ;User-Defined Layers:
  ...
  ( via9                      28         via9         )
  ( metal10                   29         metal10      )
  ( topelectrode              30         TopElectrode      )
  ( bottomelectrode           31       BottomElectrode   )
  ( metaloxide                32        MetalOxide        )
  ( nodrc                     80         nodrc        )
 ...
 ;( ---------    -------      ------          --- --- --------- ------- ----- )
  ...
  ( contact      grid         defaultPacket    t t t t t )
  ( metal1       grid         defaultPacket    t t t t t )
  ( via1         grid         defaultPacket    t t t t t )
  ( metal2       grid         defaultPacket    t t t t t )
  ( via2         grid         defaultPacket    t t t t t )
  ( metal3       grid         defaultPacket    t t t t t )
  ( via3         grid         defaultPacket    t t t t t )
  ( metal4       grid         defaultPacket    t t t t t )
  ( via4         grid         defaultPacket    t t t t t )
  ( metal5       grid         defaultPacket    t t t t t )
  ( via5         grid         defaultPacket    t t t t t )
  ( metal6       grid         defaultPacket    t t t t t )
  ( via6         grid         defaultPacket    t t t t t )
  ( metal7       grid         defaultPacket    t t t t t )
  ( via7         grid         defaultPacket    t t t t t )
  ( metal8       grid         defaultPacket    t t t t t )
  ( via8         grid         defaultPacket    t t t t t )
  ( metal9       grid         defaultPacket    t t t t t )
  ( via9         grid         defaultPacket    t t t t t )
  ( metal10      grid         defaultPacket    t t t t t )
...
viaDefs(
 ...
 ;( ---------- ------- -------- -------- ------ ------ ----------------)
  ( M2_M1_via  RERAM      M2_M1_via  via        metal1     metal2     0.0)
  ( M3_M2_via  RERAM      M3_M2_via  via        metal2     metal3     0.0)
  ( M4_M3_via  RERAM      M4_M3_via  via        metal3     metal4     0.0)
  ( M5_M4_via  RERAM      M5_M4_via  via        metal4     metal5     0.0)
  ( M6_M5_via  RERAM      M6_M5_via  via        metal5     metal6     0.0)
  ( M7_M6_via  RERAM      M7_M6_via  via        metal6     metal7     0.0)
  ( M8_M7_via  RERAM      M8_M7_via  via        metal7     metal8     0.0)
  ( M9_M8_via  RERAM      M9_M8_via  via        metal8     metal9     0.0)
  ( M10_M9_via RERAM      M10_M9_via via        metal9     metal10    0.0)
  ( M2_M1_viaB RERAM      M2_M1_viaB via        metal1     metal2     0.0)
  ( M2_M1_viaC RERAM      M2_M1_viaC via        metal1     metal2     0.0)
  ( M3_M2_viaB RERAM      M3_M2_viaB via        metal2     metal3     0.0)
  ( M3_M2_viaC RERAM      M3_M2_viaC via        metal2     metal3     0.0)
  ( M4_M3_viaB RERAM      M4_M3_viaB via        metal3     metal4     0.0)
 ) ;customViaDefs
...
constraintGroups(
 ...
 ;( -----	---------- )
  ( "foundry"	nil
    ...
    orderedSpacings(
     ( minExtensionDistance       "nimplant"	"active"		0.0 )
     ( minExtensionDistance       "pimplant"	"active"		0.0 )
     ( minExtensionDistance       "nwell"	"active"		0.055 )
     ( minExtensionDistance       "pwell"	"active"		0.055 )
     ( minExtensionDistance       "nimplant"	"contact"		0.005 )
     ( minExtensionDistance       "pimplant"	"contact"		0.005 )
     ( minExtensionDistance       "active"	"contact"		0.005 )
     ( minExtensionDistance       "poly"	"contact"		0.005 )
     ( minExtensionDistance       "metal1"	"contact"		0.0 )
     ( minExtensionDistance       "metal1"	"via1"		0.0 )
     ( minExtensionDistance       "metal2"	"via1"		0.0 )
     ( minExtensionDistance       "nwell"	"active"		0.0 )
     ( minExtensionDistance       "pwell"	"active"		0.0 )
     ( minExtensionDistance       "metal2"	"via2"		0.0 )
     ( minExtensionDistance       "metal3"	"via2"		0.0 )
     ( minExtensionDistance       "metal3"	"via3"		0.0 )
     ( minExtensionDistance       "metal4"	"via3"		0.0 )
     ( minExtensionDistance       "metal4"	"via4"		0.0 )
     ( minExtensionDistance       "metal5"	"via4"		0.0 )
     ( minExtensionDistance       "metal5"	"via5"		0.0 )
     ( minExtensionDistance       "metal6"	"via5"		0.0 )
     ( minExtensionDistance       "metal6"	"via6"		0.0 )
     ( minExtensionDistance       "metal7"	"via6"		0.0 )
     ( minExtensionDistance       "metal7"	"via7"		0.0 )
     ( minExtensionDistance       "metal8"	"via7"		0.0 )
     ( minExtensionDistance       "metal8"	"via8"		0.0 )
     ( minExtensionDistance       "metal9"	"via8"		0.0 )
     ( minExtensionDistance       "metal9"	"via9"		0.0 )
     ( minExtensionDistance       "metal10"	"via9"		0.0 )
    ) ;orderedSpacings
